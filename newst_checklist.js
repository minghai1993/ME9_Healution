...
// }
// function handleTableClick(i) {
//     var finalPdf = document.getElementById("finalPdf")
//     var addrow = document.createElement('h3');
//     finalPdf.appendChild(addrow)
//     addrow.setAttribute("id",i)
//     var text = document.getElementById(i * 100 +1).innerHTML;
...
//     var deleterow = document.getElementById(i * 100 + 1)
//     $("#" + i * 10).show()
//     $("#" + i * 100).hide()
// }
...
function generateChecklist(addd,name,q,arr){
    jQuery.get(
                    ...
                    function (result) {
                        ...
                        check.setAttribute("class","table table-striped")
                        check.setAttribute("onclick","handleIput(" + q + ")")
                        // check.setAttribute("id","resultArea")
                        console.log(arr)
                        console.log(q)
                        var addr = result.results[0].formatted_address
                        // var checkk = "<table id ='"+ (q + 1) * 100 +"' class='table table-striped'><tr onclick='handleIput(" + q + ")'><td>Go to " + arr[q].type_id + " called " + name + " at " +addr +"</td><td onclick = handleCheckDelete(" + (q + 1) + ")>Remove<ion-icon name='close'></ion-icon></td></tr></table>"
                        // // return address
                        var checkk = "<td>Go to " + arr[q].type_id + " called " + name + " at " +addr +"</td><td id='remove' onclick = handleCheckDelete(" + (q + 1) + ")>Remove<ion-icon name='close'></ion-icon></td>"
                        // return address
...
function printResult(arr) {
    var checkLlist = document.getElementById("checkelist")
    var checkelists = "<table id ='printCheck' class='table table-striped'><tr><th>Checklist</th><th><button id = 'cmd' onclick='printPdf();return false'>Download</button></th></tr></table>"
    checkLlist.innerHTML = checkelists;
    checkList = document.getElementById("printCheck")
