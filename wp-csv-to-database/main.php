...
class wp_csv_to_db {
	...
	if ( $getTable ) {
	    $result		 = $mydb->get_results( "SELECT * FROM $getTable" );
	    $requestedTable	 = $mydb->get_results( "SHOW COLUMNS FROM " . $getTable );
	...
	if ( ! empty( $_POST[ 'delete_db_button_hidden' ] ) ) {
	    ...
	    $del_qry_success = $mydb->query( $del_qry );
	...
	if ( isset( $_POST[ 'execute_button' ] ) ) {
		...
		if ( isset( $_POST[ 'remove_autoinc_column' ] ) ) {
		    $db_cols = $mydb->get_col( "DESC " . $_POST[ 'table_select' ], 0 );
		    unset( $db_cols[ 0 ] );  // Remove first element of array (auto increment column)
		}
		// Else we just grab all columns
		else {
		    $db_cols = $mydb->get_col( "DESC " . $_POST[ 'table_select' ], 0 );  // Array of db column names
		}
		...
		<div id="tabs-1">
			...
			<table class="form-table"> 
				...
				<td>
					...
					$mydb = new mydb('map_access','Minghai*8','map_db_it2','localhost');
					$sql		 = "SHOW TABLES";
...
function wp_csv_to_db_get_columns_callback() {
	...
	$sql = 'SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = "' . $mydb->dbname . '" AND TABLE_NAME ="' . $table_name . '" AND EXTRA like "%auto_increment%"';

	// Execute Query
	$run_qry = $mydb->get_results( $sql );
	...
	if ( (isset( $run_qry[ 0 ]->EXTRA )) && (isset( $run_qry[ 0 ]->COLUMN_NAME )) ) {
	    ...
	    $column_names	 = $mydb->get_col( 'DESC ' . $table_name, 0 );
	    $counter	 = 0;
